...
import os, re, pathlib, yaml, tempfile
...
def _write_controllers_yaml():
    """
    Write controller config to a temp file.
    ROS2 Node parameters cannot accept list-of-dicts inline —
    must be loaded from a yaml file.
    """
    config = {
...
def generate_launch_description():
    ...
    # ── Controllers yaml (must be a file, not inline dict) ─────────────
    controllers_yaml_path = _write_controllers_yaml()

    # ── 1. Hardware bridge (SDK + /joint_states + FollowJointTrajectory)
    hw_node = Node(
    ...
    if 'moveit' in get_packages_with_prefixes():
        ...
        move_group = Node(
            ...
            parameters=[
                ...
                ompl,
                # Controller manager type (scalar string — safe inline)
                {
                    'moveit_controller_manager':
                        'moveit_simple_controller_manager/MoveItSimpleControllerManager',
                },
                # Controller list loaded from yaml file (list-of-dicts — must be file)
        ...
        # hw_node needs ~3s for SDK init before MoveIt2 subscribes to /joint_states
