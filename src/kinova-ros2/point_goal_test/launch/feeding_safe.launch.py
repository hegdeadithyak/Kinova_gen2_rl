#!/usr/bin/env python3
"""Launch the click-to-move demo with the C++ safety-focused arm mover.

Architecture:
  point_selector_node  (Python) — clicks → /goal_point
  feeding_safe_mover   (C++)    — /goal_point → MoveIt 2 constrained planning

The Python arm_mover_node is intentionally NOT started.
Use point_goal.launch.py if you need the legacy Python mover.
"""
...
def _load_text(pkg, rel):
    return pathlib.Path(
        os.path.join(get_package_share_directory(pkg), rel)
    ).read_text()
...
def _launch_setup(context, *args, **kwargs):
    if "moveit" not in get_packages_with_prefixes():
        return [LogInfo(msg='WARNING: "moveit" not installed — aborting launch')]
    ...
    # EE → camera_color_optical_frame hand-eye TF (calibrated values)
    nodes.append(Node(
        package="tf2_ros",
        executable="static_transform_publisher",
    ...
    try:
        ...
        nodes.append(LogInfo(msg="WARNING: realsense2_camera not found — camera disabled"))

    # Python point selector (unchanged from the original demo)
    nodes.append(Node(
        package="point_goal_test",
        executable="point_selector_node",
        name="point_selector_node",
        output="screen",
        parameters=[{
            "cam_offset_x": float(context.perform_substitution(
                LaunchConfiguration("cam_offset_x"))),
            "cam_offset_y": float(context.perform_substitution(
                LaunchConfiguration("cam_offset_y"))),
        }],
    ))

    # C++ safety mover — replaces arm_mover_node
    nodes.append(Node(
        package="point_goal_test",
        executable="feeding_safe_mover",
        name="feeding_safe_mover",
        output="screen",
        parameters=[{
            "base_frame":             "root",
            "ee_link":                "j2s6s200_end_effector",
            "joint_5":                "j2s6s200_joint_5",
            "joint_6":                "j2s6s200_joint_6",
            ...
            "enable_joint_constraints": False,
        }],
    ))
...
def generate_launch_description():
    return LaunchDescription([
        # Hand-eye calibration (EE → camera_color_optical_frame)
